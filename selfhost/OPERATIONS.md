...
The page separates Overview, Branding, Email, Integrations, Bakney pairing,
...
Collegamento Bakney**, generate a pairing secret, and copy it together with the
...
`BAKNEY_SSO_AUTHORITY` defaults to `https://app.bakney.com`. A URL change requires
...
Existing local sessions continue until their normal expiration. Bakney must
implement the [version 1 pairing and login contract](../docs/contracts/bakney-sso-v1.md)
before the feature can be enabled. That document includes endpoint schemas,
