<script>
    ...
    onMount(() => {
        ...
        apiFetch(__bakney.env.API.BILLING.CHECKOUT, {method: 'POST'}).then(res => {
            if (!res.error) {
...
<div style="margin: auto; text-align: center;">
    <div class="mb-10">
        ...
        {:else}
            ...
            <h2 class="display-4 text-center text-dark-75 font-weight-bolder py-3">
                <img src="/static/stripe_watermark.png" alt="" style="max-width: 50%; width: 100px;" /> <br />
                Si sono verificati degli errori nel completare l'attivazione del piano! Provare ad aggiornare la pagina.<br />
                {#if $oemConfig?.supportEmail}
                    Se il problema persiste contattare l'assistenza tecnica all'indirizzo
                    <a href="mailto:{$oemConfig.supportEmail}">{$oemConfig.supportEmail}</a>
                {/if}
            </h2>
        {/if}
    </div>
