<script>
    ...
    async function save() {
        ...
        try {
            ...
            feedback = 'Impostazioni salvate. I nuovi invii da API e worker usano questa configurazione senza riavvio.';
            onSaved();
        } catch (e) { error = e.message; }
        finally { busy = false; }
    }
    async function resetEnvironment() {
        ...
        try {
            ...
            feedback = 'Ripristinata la configurazione di ambiente. Nessuna variabile sul server è stata modificata.';
            onSaved();
        } catch (e) { error = e.message; }
        finally { busy = false; }
    }
    async function test(action) {
        if (busy || disabled || changes) return;
        busy = true; error = ''; feedback = ''; testResult = null;
        try {
...
<section aria-labelledby="instance-email-title">
    ...
    {#if error}<p class="alert alert-danger" role="alert">{error}</p><button class="btn btn-light mb-4" disabled={busy || changes} on:click={load}>Ricarica configurazione</button>{/if}
    {#if feedback}<p class="alert alert-success" role="status">{feedback}</p>{/if}
    {#if config}
        ...
        {#if config.credential_error}<p class="alert alert-warning">La password salvata non può essere letta. Reinseriscila o rimuovila esplicitamente.</p>{/if}
        <form on:submit|preventDefault={save}>
            <fieldset disabled={disabled || busy || loading}>
                ...
                <details class="mt-4">
                    ...
                    <label class="d-block mt-3"><input type="checkbox" bind:checked={clearPassword} on:change={() => {if (clearPassword) password = '';}} /> Rimuovi la password salvata</label>
        ...
        {#if config.source === 'instance'}
            <details class="mt-6">
                ...
                <label class="d-block"><input type="checkbox" bind:checked={confirmReset} disabled={busy || disabled || changes} /> Confermo il ripristino della configurazione di ambiente</label>
