...
<div
    ...
    <div>
        <input
            style="position:absolute;opacity:0!important;pointer-events:none;"
            type="text"
        ...
        {#if !editable}
            <SmoothSignature
                on:update={e => {
                    if (e.detail.there_is_signature) {
                        props.value = e.detail.data;
                    } else {
                        props.value = null;
                    }
                }} />
        {:else}
            <div>
                <h6 class="text-center text-dark font-weight-boldest text-primary">Anteprima del componente</h6>
                <div style="pointer-events:none;opacity:0.4;filter: blur(1px);">
                    <SmoothSignature bind:signature={props.value} />
                </div>
            </div>
        {/if}
    </div>
