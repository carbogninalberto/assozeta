...
const status = () => ({available: true, protocol: 1, is_owner: true, can_update: stage !== 'recovery_required', active: stage === 'backup' ? operation() : null, history: stage && stage !== 'backup' ? [operation()] : []});
...
export async function apiFetch(url, options = {}) {
    ...
    return {error: !response.ok, response: await response.json()};
}`;
try {
    ...
    const api = createServer(async (req, res) => {
        ...
        if (req.url === '/instance-update-status') return send(status());
